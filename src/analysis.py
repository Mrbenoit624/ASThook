...
def Manifest(app):
    global package
    tree = ET.parse('%s/%s/%s/AndroidManifest.xml' % (DIR, "decompiled_app",
        app.split('/')[-1][:-4]))
    root = tree.getroot()
...
def UserInput(app):
    for path in Path('%s/decompiled_app' % DIR).rglob('*.java'):
        print(path)
...
def static(app):
    ...
    subprocess.call(["python3", "src/submodule/apk2java-linux/apk2java.py", app,
    "--java", "-o", "%s/decompiled_app" % DIR],
            stdout=STD_OUTPOUT, stderr=STD_ERR, shell=False)
    Manifest(app)
    UserInput(app)
